...
function QuizScreen({navigation}) {
    ...
    const handlePress = (value) => { //Picks out the one they clicked on, setting its index in values[0] to 'update'.
    ...
    const buttonPress = () => {
        ...
        if (counter == values[0].length - 5) {      //If the counter gets to five less than the total number of words, 
        ...
        for (let i = 0; i < pointers.length; i++) { 
            if (temp[i] == update) {                //Whichever pointer is pointing at the value the user selected...
                        ...
                        <View style = {styles.appPageContainer}>
                            <View style = {styles.headerContainer}>
                                <Text style={styles.header}>
                                    Choose the option you care about LEAST from the
                                    values below:
                                </Text>
                            </View>
                            ...
                            <View>
                                <Text onPress={buttonPress} style={styles.button}>
                                    Continue
                                </Text>
                                <Text onPress={returnHome} style={styles.button}>
                                    Home
                                </Text>
                            </View>
                        </View>
                    ) : (
                        <Text style = {styles.announcement}>The quiz information is downloading.</Text>
                    )}
                </View>
            )}
        </View>
